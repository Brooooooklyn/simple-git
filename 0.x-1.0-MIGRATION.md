...
> guard that treated `0` as "missing" now passes — test `finalAuthorName` /
> `finalAuthorEmail` (still `null` when absent) instead, or compare the timestamp.
...
- **`dispose()` / `free()` (new).** Release the native repo handle eagerly (frees the
  ...
  `Repository`'s `Option`-returning lookups — `workdir()`, `namespace()`,
  ...
  guard post-dispose use yourself. (2) A previously-obtained `RevWalk` / `TreeIter` /
...
- **`fetchAsync` / `pushAsync` are not drop-in for `fetch` / `push`.** Their
  ...
  **not** return a rejected promise), so wrap the call, not just the awaited promise.
  ...
  the underlying git2 handle is not `Sync`. `fetchAsync` re-resolves the
