...
- **`fetchAsync` / `pushAsync` are not drop-in for `fetch` / `push`.** Their
  ...
  **not** return a rejected promise), so wrap the call, not just the awaited promise.
  ...
  `fetch()`/`push()`. `pushAsync` additionally throws synchronously when the remote's
  push URL is absent/unreadable or its configured push refspecs can't be read. **Do not
