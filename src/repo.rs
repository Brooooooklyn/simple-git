...
impl Repository {
  ...
  ///
  /// After disposal every method on this `Repository` throws
  /// `"Repository has been disposed"`. Any handle previously derived from this
  /// repository — `Remote`, `Reference`, `Tree`, `TreeEntry`, `Commit`, `Tag`,
  /// `Branch`, `GitObject`, `Diff`, `RevWalk` and their descendants — is also
  /// INVALID and must not be used. This is a documented contract (mirroring
  /// better-sqlite3's `db.close()`), not a machine-enforced guarantee.
  ///
