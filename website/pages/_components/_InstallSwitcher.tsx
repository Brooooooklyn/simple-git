...
// Package-manager tabs. Commands are the exact install lines for @napi-rs/simple-git.
const PMS = [
  { id: 'npm', cmd: 'npm install @napi-rs/simple-git' },
  { id: 'yarn', cmd: 'yarn add @napi-rs/simple-git' },
  { id: 'pnpm', cmd: 'pnpm add @napi-rs/simple-git' },
  { id: 'bun', cmd: 'bun add @napi-rs/simple-git' },
]

const STORAGE_KEY = 'napi-simple-git:pm'

// Interactive island (tabs + localStorage-persisted choice + copy button). Hydrates
// automatically as part of the regular route's full-page hydration. SSR renders the
// `npm` tab, so a working install command is visible with JS off.
