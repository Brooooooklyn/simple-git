...
Every error thrown by this library — from a synchronous method or a rejected `*Async` promise — is a standard `Error` carrying a `code` string drawn from the **`GitErrorCode`** const enum. Narrow any caught value with the total **`isGitError`** type guard:
